...
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16MB max file size akhon rakhlam
...
def init_db():
    ...
    try:
        ...
        # Deleted messages track korar jonno notun table
        c.execute("""
...
# ----------------- Enhanced Caching System bujhle?-----------------
class EnhancedCache:
...
# ----------------- Main Super App Template bujhle? -----------------
...
def api_delete_message():
    try:
        ...
        print(f"API Delete Request: message_id={message_id}, user={user_phone}, type={delete_type}")
        ...
        if isinstance(message_id, str) and message_id.startswith('temp_'):
            print(f"Temporary ID, handling via socket only: {message_id}")
            return jsonify({
                "success": True, 
                "message": "Temporary message will be deleted via socket",
                "temp_id": message_id
            })
        ...
        try:
            ...
            print(f"Message found: sender={sender}, receiver={receiver}")

            if delete_type == "for_me":
                # sudhu nijer kache delete korte parbe
                ...
                print(f"Tracked deletion in deleted_messages table")

            elif delete_type == "for_everyone":
                # sender sobar jonno delete korte parbe
                if user_phone == sender:
        ...
        # FIXED: Clear ALL cache patterns for BOTH users
        ...
        # Clear all message cache for this conversation
        cache.clear_pattern(f"messages_{sender}")
        cache.clear_pattern(f"messages_{receiver}")
        print(f" Cleared ALL cache for both users")
...
def api_get_messages():
    ...
    try:
        ...
        try:
            c = conn.cursor()
            # FIXED: Properly filter deleted messages for real-time delete
            c.execute("""
                ...
                AND (
                    (m.deleted_for_everyone = 0) AND
                    NOT (m.sender = ? AND m.deleted_for_sender = 1) AND
                    NOT (m.receiver = ? AND m.deleted_for_receiver = 1)
                )
...
<body>
    ...
    <script>
        ...
        function showDeleteOptionsFromContext() {
            ...
            console.log(' Delete options for message:', currentMessageToDelete);
        }
        ...
        function loadMessages() {
            fetch(`/api/get_messages?user_phone=${encodeURIComponent(myPhone)}&contact_phone=${encodeURIComponent(contactPhone)}&page=1&limit=50`)
                ...
                .then(messages => {
                    ...
                    messages.forEach(m => {
                        console.log(' Message:', m.id, m.sender, m.message.substring(0, 20));
                        if (!m.is_deleted) {
            ...
            if (socket.connected) {
                socket.emit('join', {
                    ...
                    if (ack && ack.success) {
                        ...
                        console.log(' Retrying to join room...');
        ...
        // Handle immediate delete events for TEMP messages
        socket.on('immediate_delete', function(data) {
            console.log('IMMEDIATE DELETE EVENT:', data);
            
            const tempId = data.temp_id;
            const messageElement = document.querySelector(`[data-message-id="${tempId}"]`);
            
            if (messageElement) {
                if (data.delete_type === 'for_me' && data.deleted_by === myPhone) {
                    // Only remove for the user who deleted it
                    messageElement.remove();
                    console.log(' Removed temp message for me:', tempId);
                    ...
                    console.log('Marked temp message as deleted for everyone:', tempId);
                }
            }
        });

        // Handle INSTANT delete events (immediate UI update)
        socket.on('instant_message_deleted', function(data) {
            console.log('INSTANT MESSAGE DELETED EVENT:', data);
            ...
            console.log('Details:', {
                ...
                deletedBy,
                myPhone: myPhone,
                isForMe: deleteType === 'for_me' && deletedBy === myPhone,
                isForEveryone: deleteType === 'for_everyone'
            });
            ...
            if (messageElement) {
                if (deleteType === 'for_me') {
                    // If user deleted for themselves, remove from THEIR view only
                    if (deletedBy === myPhone) {
                        messageElement.remove();
                        console.log(' INSTANT: Removed message from UI (deleted for me)');
                    }
                } 
                else if (deleteType === 'for_everyone') {
                    // Show deleted message for everyone IMMEDIATELY
                    messageElement.classList.add('deleted-message');
                    ...
                    if (messageContent) {
                        ...
                        messageContent.style.fontStyle = 'italic';
                        messageContent.style.color = '#999';
                        messageContent.style.pointerEvents = 'none';
                    }
                    ...
                    // Remove context menu capability
                    messageElement.style.pointerEvents = 'none';
                    
                    console.log(' INSTANT: Updated message to show as deleted for everyone');
                }
            } else {
                console.log('Message element not found for ID:', messageId);
                
                // Try to find temporary message
                if (messageId.toString().startsWith('temp_')) {
                    const tempElement = document.querySelector(`[data-message-id="${messageId}"]`);
                    if (tempElement) {
                        tempElement.remove();
                        console.log(' Removed temporary message:', messageId);
                    }
                }
            }
            
            // Force clear cache and reload contacts
            clearLocalCache();
            
            // If message was deleted for everyone, clear local storage too
        ...
        // Background e click korle model off hobe
        ...
        function deleteMessageForMe() {
            ...
            console.log(' Deleting message for me:', currentMessageToDelete);

            // Check if it's a temp message
            if (currentMessageToDelete.toString().startsWith('temp_')) {
                // For temp messages, emit immediate delete event
                socket.emit('immediate_delete', {
                    temp_id: currentMessageToDelete,
                    user_phone: myPhone,
                    delete_type: 'for_me',
                    sender: myPhone,
                ...
                console.log('Emitted immediate delete for temp message');
            ...
            .then(data => {
                if (data.success) {
                    ...
                    //  FIXED: Emit socket event for REAL-TIME delete
                    ...
                    console.log('Emitted delete_message socket event');
        ...
        function deleteMessageForEveryone() {
            ...
            console.log('Deleting message for everyone:', currentMessageToDelete);
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // FIXED: Emit socket event for REAL-TIME delete for everyone
                    socket.emit('delete_message', {
                        message_id: currentMessageToDelete,
                        user_phone: myPhone,
                        delete_type: 'for_everyone',
                    ...
                    console.log('Emitted delete_message socket event for everyone');
        ...
        //  FIXED: Handle delete_message socket event
        socket.on('delete_message', function(data) {
            console.log(' DELETE MESSAGE SOCKET EVENT:', data);
            ...
            if (messageElement) {
                if (deleteType === 'for_me') {
                    // If the current user deleted for themselves
                    if (deletedBy === myPhone) {
                        messageElement.remove();
                        console.log(' Removed message from UI (deleted for me)');
                    }
                } 
                else if (deleteType === 'for_everyone') {
                    // Show deleted message for everyone
                    messageElement.classList.add('deleted-message');
                    ...
                    console.log('Updated message to show as deleted for everyone');
                }
            }
        ...
        //  NEW FUNCTION: Clear all local cache
        function clearLocalCache() {
            // Clear localStorage cache
            const keysToRemove = [];
            for (let i = 0; i < localStorage.length; i++) {
                const key = localStorage.key(i);
                if (key.includes('messages') || key.includes('contacts')) {
            ...
            console.log(' All local cache cleared');
        }
        ...
        function sendMessage() {
            ...
            const tempId = 'temp_' + Date.now();
            addMessage(String(myPhone), msg, 'sent', tempId, true, []);
        ...
        console.log('UI updates IMMEDIATELY - No waiting!');
        console.log('Both users see delete instantly!');
        console.log('FIXED Context Menu System - Single tap will NOT open context menu!');
        console.log('Context menu only opens on right-click or long-press (500ms)');
        console.log('RIFRESH NA KORLEO DELETE DEKHABE!');
    </script>
</body>
</html>"""
...
def get_room(user, contact):
    """Thik vabe room banao go devoloper"""
    try:
        # Ensure both are strings
        user = str(user).strip()
        contact = str(contact).strip()
        
        # Sort to ensure consistent room name
        users = [user, contact]
...
def on_join(data):
    try:
        ...
        print(f"User {user} joined room {room} for chat with {contact}")
...
def handle_message(data):
    try:
        ...
        emit('receive_message', {'id': message_id, 'sender': sender, 'message': message}, room=room, broadcast=True)
...
# ----------------- FIXED: DELETE MESSAGE SOCKET EVENT -----------------
@socketio.on('delete_message')
def handle_delete_message(data):
    """Handle delete message events from clients"""
    try:
        message_id = data.get('message_id')
        user_phone = data.get('user_phone')
        ...
        print(f"DELETE MESSAGE SOCKET EVENT: message_id={message_id}, user={user_phone}, type={delete_type}")
        ...
        print(f" Broadcasted delete event to room {room}")

    except Exception as e:
        print(f" Error in handle_delete_message: {e}")
        emit('error', {'message': 'Failed to broadcast delete'})

@socketio.on('immediate_delete')
def handle_immediate_delete(data):
    """Handle immediate delete events for temp messages"""
    try:
        ...
        print(f" Immediate delete broadcasted: {temp_id}")
        
    except Exception as e:
        print(f" Error in immediate_delete: {e}")
...
def on_disconnect():
    try:
        ...
        print(f" A sala beriye galo: {request.sid}")
    except Exception as e:
...
# ----------------- choto kore Security Information  -----------------
@app.route("/security")
def security_info():
    return """
    <!DOCTYPE html>
    ...
    <body>
        <div class="container">
            ...
            <div class="feature">
                ...
                <p>Delete messages for yourself or for everyone with WhatsApp-style INSTANT deletion.</p>
            </div>
