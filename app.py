...
<body>
  ...
  <script>
    ...
    function loadContacts(phone) {
      console.log("📞 Loading contacts for:", phone);
        ...
        .catch(error => {
          ...
          content.innerHTML = `
            ...
            <div class="placeholder-content">
              ...
              <button onclick="loadContacts('${phone}')" style="background: #0E4950; color: white; border: none; padding: 10px 20px; border-radius: 8px; cursor: pointer; margin-top: 10px;">
                Retry
              </button>
            </div>
          `;
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      console.log("📱 Logged in phone from URL:", loggedInPhone);
      console.log("💾 Phone from localStorage:", localStorage.getItem('exomnia_user_phone'));
      
      if (loggedInPhone) {
        console.log("✅ Setting phone to localStorage:", loggedInPhone);
        localStorage.setItem('exomnia_user_phone', loggedInPhone);
        
        // Remove the parameter from URL
        const newUrl = window.location.pathname;
        window.history.replaceState({}, '', newUrl);
        console.log("🔄 URL cleaned, new URL:", newUrl);
        
        // Open chat tab and load contacts
        openTab('chat');
        
        // Small delay to ensure DOM is ready
        setTimeout(() => {
          loadContacts(loggedInPhone);
        }, 100);
      } else {
        ...
        if (savedPhone) {
          ...
          // Open chat tab by default
          openTab('chat');
          
          // Small delay to ensure DOM is ready
          setTimeout(() => {
            loadContacts(savedPhone);
          }, 100);
        } else {
          console.log("❌ No user logged in");
          // Open chat tab with login prompt
          openTab('chat');
        }
      }
    });
