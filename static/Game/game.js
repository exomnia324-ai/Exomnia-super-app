...
let G={};
function initG(){
  const _keepModeCfg=(typeof G!=='undefined'&&G&&G._modeCfg)?G._modeCfg:MODES.normal;
  const _keepMode=(typeof G!=='undefined'&&G&&G._selectedMode)?G._selectedMode:'normal';
  G={
    _modeCfg:_keepModeCfg,_selectedMode:_keepMode,
...
const BIOMES=[
  {name:'DEEP SPACE',      bgIn:'#00050f',bgMid:'#000308',bgOut:'#000104',
   ...
   starTint:['#ffffff','#cce8ff','#ffd8b0'],asteroidDensity:.5,asteroidCol:'#6b6459',fogCol:null,hazard:'rock'},
  {name:'ASTEROID FIELD',  bgIn:'#0f0906',bgMid:'#080503',bgOut:'#040201',
   nc:['rgba(110,60,0','rgba(90,40,10','rgba(70,50,20','rgba(120,70,0'],
   starTint:['#ffe8cc','#ffd8b0','#ffffff'],asteroidDensity:1.6,asteroidCol:'#8a7a5f',fogCol:null,hazard:'rock'},
  {name:'NEBULA STORM',    bgIn:'#140020',bgMid:'#0a0014','bgOut':'#05000a',
   nc:['rgba(150,0,180','rgba(90,0,160','rgba(180,0,120','rgba(60,0,140'],
   starTint:['#e8ccff','#ff99ee','#ffffff'],asteroidDensity:.3,asteroidCol:'#7a5a8a',fogCol:'rgba(150,0,200,0.05)',hazard:'rock'},
  {name:'CRIMSON WASTES',  bgIn:'#170303',bgMid:'#0c0101',bgOut:'#050000',
   nc:['rgba(160,10,10','rgba(120,20,0','rgba(100,0,30','rgba(180,40,0'],
   starTint:['#ffcccc','#ff9999','#ffffff'],asteroidDensity:.9,asteroidCol:'#ff6a33',fogCol:'rgba(180,20,10,0.04)',hazard:'mine'},
  {name:'ICE EXPANSE',     bgIn:'#02121a',bgMid:'#010a0f',bgOut:'#000507',
   nc:['rgba(0,90,150','rgba(0,140,180','rgba(20,60,120','rgba(0,170,200'],
   starTint:['#cceeff','#ffffff','#aaddff'],asteroidDensity:1.1,asteroidCol:'#bfe9ff',fogCol:'rgba(0,150,220,0.05)',hazard:'shard'},
  {name:'DERELICT RUINS',  bgIn:'#0a0a0a',bgMid:'#050505',bgOut:'#020202',
   nc:['rgba(60,80,60','rgba(40,60,40','rgba(80,90,60','rgba(50,70,50'],
   starTint:['#ccffcc','#ffffff','#aaffaa'],asteroidDensity:.7,asteroidCol:'#5a6a5a',fogCol:'rgba(60,90,60,0.04)',hazard:'wreck'},
];
function getBiome(){return BIOMES[Math.floor((G.wave-1)/3)%BIOMES.length];}

/* ── MISSION MODES ──
   Wired to the (previously cosmetic) NORMAL / SURVIVAL / BLITZ buttons on the
   lobby screen. Each mode changes real pacing/difficulty/scoring, not just a label. */
const MODES={
  normal:  {label:'NORMAL',   waveCap:10,  hpMult:1,    spdMult:1,    spawnCap:4, scoreMult:1,
            announce:'◈ MODE: NORMAL — CLEAR 10 WAVES TO WIN'},
  survival:{label:'SURVIVAL', waveCap:null,hpMult:1.3,  spdMult:1.15, spawnCap:4, scoreMult:1.2,
            announce:'◈ MODE: SURVIVAL — ENDLESS, TOUGHER ENEMIES'},
  blitz:   {label:'BLITZ',    waveCap:null,hpMult:.7,   spdMult:1.2,  spawnCap:6, scoreMult:2,
            announce:'◈ MODE: BLITZ — 2× SCORE, FASTER & THINNER ENEMIES'},
};
function curModeCfg(){return G._modeCfg||MODES.normal;}
...
function spawnAsteroid(){
  const bm=getBiome();
  const hz=bm.hazard||'rock';
  // MINES (Crimson Wastes) — 1-hit-pops, drifts slow, punishes contact hard so
  // players learn to snipe them from a distance instead of just dodging.
  if(hz==='mine'){
    const r=rnd(16,25);
    G.asteroids.push({x:rnd(r,CV.width-r),y:-r-20,r,hp:1,maxHp:1,
      dy:rnd(.6,1.1),dx:rnd(-.3,.3),rot:rnd(0,6.28),rotSpd:rnd(-.05,.05),
      col:bm.asteroidCol,big:false,seed:Math.random()*99,hazard:'mine'});
    return;
  }
  // WRECKAGE (Derelict Ruins) — big, nearly-stationary hull chunks. Hard to
  // destroy, so the field forces you to actually navigate around them.
  if(hz==='wreck'){
    const r=rnd(36,56);
    G.asteroids.push({x:rnd(r,CV.width-r),y:-r-20,r,hp:Math.round(r*7),maxHp:Math.round(r*7),
      dy:rnd(.22,.4),dx:0,rot:rnd(0,6.28),rotSpd:rnd(-.006,.006),
      col:bm.asteroidCol,big:true,seed:Math.random()*99,hazard:'wreck'});
    return;
  }
  // ICE SHARDS (Ice Expanse) — lighter contact sting than rock, but denser
  // fields, so it reads as a different kind of obstacle field, not palette-swapped rocks.
  const isShard=hz==='shard';
  const big=Math.random()<(isShard?.16:.28);
  const r=big?rnd(30,48):rnd(isShard?9:12,isShard?19:24);
...
function updateAsteroids(f,dt){
  ...
  for(let i=G.asteroids.length-1;i>=0;i--){
    ...
    if(a.y>CV.height+a.r+30||a.hp<=0){
      if(a.hp<=0){
        const bCol=a.hazard==='mine'?'#ff5522':a.hazard==='shard'?'#bde8ff':a.hazard==='wreck'?'#8a9a7a':(a.big?'#c8a878':'#a89868');
        burst(a.x,a.y,bCol,a.big?16:(a.hazard==='mine'?14:8));SFX.enemy_explode&&SFX.enemy_explode();
      }
      G.asteroids.splice(i,1);continue;
    }
    // contact damage vs player — varies by hazard type so each biome's field feels distinct
    if(G.invT<=0&&hitCircle(G.px,G.py,G.pw*.4,a.x,a.y,a.r*.85)){
      if(a.hazard==='mine'){
        hitPlayer(28);a.hp=0;burst(a.x,a.y,'#ff5522',18);shake(12,1.2);
      } else if(a.hazard==='shard'){
        hitPlayer(10);a.hp-=999;burst(a.x,a.y,'#bde8ff',8);shake(4,.6);
      } else if(a.hazard==='wreck'){
        hitPlayer(34);a.hp-=70;burst(a.x,a.y,'#8a9a7a',12);shake(16,1.3);
      } else {
...
function drawAsteroid(a){
  ...
  if(a.hazard==='mine'){
    // spiky pulsing mine — visually screams "don't touch me"
    const pulse=1+.12*Math.sin(a.seed+performance.now()*.006);
    const spikes=8;
    CX.beginPath();
    for(let i=0;i<spikes*2;i++){
      const ang=(i/(spikes*2))*Math.PI*2;
      const rr=a.r*(i%2===0?1*pulse:.55);
      CX.lineTo(Math.cos(ang)*rr,Math.sin(ang)*rr);
    }
    CX.closePath();
    CX.fillStyle=a.col;CX.fill();
    CX.strokeStyle='rgba(0,0,0,0.5)';CX.lineWidth=2;CX.stroke();
    CX.fillStyle='rgba(255,255,255,0.8)';
    CX.beginPath();CX.arc(0,0,a.r*.22,0,Math.PI*2);CX.fill();
  } else if(a.hazard==='wreck'){
    // blocky hull wreckage — jagged rectangles, not a smooth rock
    CX.fillStyle=a.col;
    CX.strokeStyle='rgba(0,0,0,0.5)';CX.lineWidth=2;
    const chunks=[[-1,-1,1.6,1.1],[.1,-.7,.9,.7],[-.9,.1,.8,.9],[.2,.2,.9,.8]];
    for(const c of chunks){
      CX.beginPath();CX.rect(c[0]*a.r*.55,c[1]*a.r*.55,c[2]*a.r*.55,c[3]*a.r*.55);CX.fill();CX.stroke();
    }
    CX.fillStyle='rgba(255,180,60,0.35)';
    CX.beginPath();CX.arc(0,0,a.r*.18,0,Math.PI*2);CX.fill();
  } else if(a.hazard==='shard'){
    // faceted ice crystal instead of a round rock
    const pts=6;
    CX.beginPath();
    for(let i=0;i<pts;i++){
      const ang=(i/pts)*Math.PI*2;
      const rr=a.r*(i%2===0?1:.5);
      const px=Math.cos(ang)*rr,py=Math.sin(ang)*rr;
      if(i===0)CX.moveTo(px,py);else CX.lineTo(px,py);
    }
    CX.closePath();
    CX.fillStyle=a.col;CX.globalAlpha=.85;CX.fill();CX.globalAlpha=1;
  ...
  CX.restore();
  // hp bar for big/tough obstacles that have taken damage
  if((a.big||a.hazard==='wreck')&&a.hp<a.maxHp){
    const w=a.r*1.6;
...
function update(){
  ...
  if((!G.coopMode||G.coopIsHost)&&!G.bossOn&&G.enemies.length<(curModeCfg().spawnCap||4)&&G.wSpawned<G.wMax)spawnEnemy();
...
function bossShoot(){
  ...
  if(kit===1){
    ...
    // MINE DROP — slow, heavy shells dropped in a spread; big threat area, easier to read/dodge
    const count=G.bossPhase===3?6:G.bossPhase===2?4:3;
    for(let i=0;i<count;i++){
      const ox=(i-(count-1)/2)*40;
      G.eBullets.push({x:G.bossX+ox,y:G.bossY+50,dx:rnd(-.3,.3),dy:sp*.45,dmg:G.bossPhase===3?28:20});
    }
  } else if(kit===4){
    // TELESTRIKE — boss blinks into the player's lane, then unloads a focused burst
    if(G.bossT%1600<30){G.bossX=clamp(G.px,110,CV.width-110);shake(8,.8);}
    const count=G.bossPhase===3?5:G.bossPhase===2?3:2;
    for(let i=0;i<count;i++){
      const a=(i-(count-1)/2)*6*Math.PI/180;
      G.eBullets.push({x:G.bossX,y:G.bossY+60,dx:(dx/d*Math.cos(a)-dy/d*Math.sin(a))*sp*1.15,dy:(dx/d*Math.sin(a)+dy/d*Math.cos(a))*sp*1.15,dmg:G.bossPhase===3?32:22});
    }
  } else if(kit===5){
    // CROSSFIRE — twin counter-rotating arms sweep the whole arena
    const count=G.bossPhase===3?4:3;
    const baseA=G.bossT*.003;
    for(let arm=0;arm<2;arm++){
      for(let i=0;i<count;i++){
        const a=baseA*(arm===0?1:-1)+(i/count)*Math.PI*2;
        G.eBullets.push({x:G.bossX,y:G.bossY+50,dx:Math.cos(a)*sp*.8,dy:Math.abs(Math.sin(a))*sp*.8+1,dmg:G.bossPhase===3?22:14});
      }
    }
  } else {
...
function addScore(pts,x,y){
  const mult=(1+Math.floor(G.combo/5)*.5)*(curModeCfg().scoreMult||1);
...
function spawnEnemy(){
  ...
  // Mission-mode scaling — Survival is genuinely tougher, Blitz trades enemy bulk for speed
  const mc=curModeCfg();
  hp=Math.round(hp*(mc.hpMult||1));
  if(type!=='turret')spd*=(mc.spdMult||1);
...
function spawnBoss(){
  ...
  const kitNames=['SPREAD BARRAGE','SWEEPING BEAM','HOMING SALVO','MINE DROP','TELESTRIKE','CROSSFIRE'];
  G.bossName=names[Math.min(Math.floor(G.wave/3)-1,names.length-1)]||'DESTROYER';
  G.bossKit=Math.max(0,Math.floor(G.wave/3)-1)%kitNames.length;
  G.bossHp=Math.round((1000+G.wave*300)*(curModeCfg().hpMult||1));G.bossMaxHp=G.bossHp;
...
function advWave(){
  ...
  const cap=curModeCfg().waveCap;
  if(cap&&G.wave>=cap){missionComplete();return;}
...
// Fires when a capped mode (currently NORMAL, 10 waves) is fully cleared —
// previously the "10 Waves" lobby label was purely cosmetic and the run just kept going forever.
async function missionComplete(){
  G.over=true;G.alive=false;
  SFX.game_over&&SFX.game_over();
  $id('endTitle').textContent='MISSION COMPLETE';$id('endTitle').className='win';
  ...
  if(typeof API!=='undefined'&&API.saveRun) await API.saveRun();
}
